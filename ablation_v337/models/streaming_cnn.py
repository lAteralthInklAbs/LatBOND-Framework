...
class StreamingCNN(nn.Module):
    ...
    Architecture:
        Input (n_mels x T) -> [optional spectral flux concat] ->
        3 x CausalConv1d blocks (64->128->128) ->
    ...
    def __init__(self, n_mels=229, n_pitches=88, channels=None,
                 kernel_size=3, dilations=None, use_spectral_flux=True):
        super().__init__()
        ...
        # Causal conv blocks with dilations
        layers = []
        for i, out_ch in enumerate(channels):
            dilation = dilations[i] if i < len(dilations) else 1
            layers.extend([
                CausalConv1d(in_ch, out_ch, kernel_size, dilation=dilation),
                nn.BatchNorm1d(out_ch),
                nn.ReLU(inplace=True),
                nn.Dropout(0.15),  # v3: increased dropout
            ])
            in_ch = out_ch
    ...
    def get_receptive_field(self):
        ...
        # 3 layers x (kernel_size - 1) x dilation + 1
...
def create_streaming_cnn(config):
    ...
    return StreamingCNN(
        ...
        dilations=getattr(config, 'cnn_dilations', [1, 2, 4, 8]),
