"""
LatBOND Trainer v3.3.7
=======================
v3.3.7 FIXES (reproducing v11's matched > truncated result):
...
class LatBONDTrainer:
    ...
    # ==================================================================
    # v3.3.4 CONTEXT PER CONDITION (NO CURRICULUM)
    # ==================================================================

    def get_context_for_epoch(self, epoch):
        """
        v3.3.4 FIX: No curriculum for matched condition.

        Matched training MUST use constrained context from epoch 1.
        The whole point of matched training is that training = deployment.
        A slow curriculum from 500 -> target makes matched ≈ truncated.

        Truncated uses full 500 frames throughout (distribution shift at deploy).
        Buffered uses 500 + budget (lookahead, upper bound, not real-time).
        """
        target_ctx = self.context_frames

        if self.condition == 'matched':
            return target_ctx
        elif self.condition == 'truncated':
            return 500
        elif self.condition == 'buffered':
    ...
    def _last_frame_constrained_eval(self, mel):
        ...
        # This is needed because stride > 1 means not every frame is evaluated
        position_set = set(positions)
        for t in range(T):
            if t not in position_set:
                # Find nearest evaluated position
                nearest_pos = min(positions, key=lambda x: abs(x - t))
                result_onset[:, :, t] = result_onset[:, :, nearest_pos]
                result_offset[:, :, t] = result_offset[:, :, nearest_pos]
                result_velocity[:, :, t] = result_velocity[:, :, nearest_pos]

        return {
            'onset_logits': result_onset,
            'offset_logits': result_offset,
            'velocity_logits': result_velocity,
        }
    ...
    def train(self, train_loader, val_loader, output_dir):
        ...
        print(f"\nCurriculum: cosine (v11)")
        print(f"Epochs: {self._max_epochs} | Patience: {self.config.early_stopping_patience}")
